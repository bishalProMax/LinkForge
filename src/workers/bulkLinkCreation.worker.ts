...
const bulkLinkCreationWorker = new Worker<BulkLinkRowJob>( "bulkLinkCreationQueue", async (job: Job<BulkLinkRowJob>): Promise<void> => {
    const { bulkOperationId, userId, row, input } = job.data;

    // Every bulk row runs through the exact same Zod schema a manual single-link submission uses — bulk never gets looser validation than the regular create form.
    const customExpiryDate = input.customExpiry ? parseFlexibleDate(input.customExpiry) : undefined;
    const parsed = createUrlSchema.safeParse({ destinationURL: input.destinationURL, customAlias: input.customAlias, title: input.title, expiration: input.expiration, customExpiry: customExpiryDate });
    ...
    try {
      const shortId = await generateShortURL({ destinationURL: parsed.data.destinationURL, userId, customAlias: parsed.data.customAlias, title: parsed.data.title, expiration: parsed.data.expiration, customExpiry: parsed.data.customExpiry });
